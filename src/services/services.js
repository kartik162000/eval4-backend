...
const getAllContentTypes = async (userId) => {
    ...
    const allContentTypes=await db.Content.findAll({where:{userId:userId}});
    if(allContentTypes!=0)
    {
        allContentTypes.forEach((contentType) => {
            allContentTypeList.push({'contentType':contentType.dataValues.contentType,id:contentType.dataValues.id});
        }
        );
...
const updateContentTypes = async (data) => {
    ...
    const dataExtracted=await db.Content.findOne({where:{contentType:contentType}});
    const existingStructure=(dataExtracted.dataValues.contentStructure);
    id(existingStructure===null)
    {
        return await db.Content.update({contentStructure:contentStructure},{where:{contentType:contentType}});
    }
    const newStructure=Object.assign(existingStructure,contentStructure);
        return await db.Content.update({contentStructure:newStructure},{where:{contentType:contentType}});
};
...
const saveCollections=async (data) => {
    ...
    const contents=await db.Content.findOne({where:{userId:userId,contentType:contentType}});
...
const getAllFieldValues = async (id,contentType) => {
    ...
    const contentStructure=allFieldValues[0]?.dataValues.Content.contentStructure;
